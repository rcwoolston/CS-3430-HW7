...
def filterFindEdges(image):
    im1 = image.filter(ImageFilter.FIND_EDGES)
    im1.save("EDGES.jpg")
...
for f in os.listdir(sharp_folder_path):
    ...
    if ext.lower() not in valid_images:
        continue
    sharp_images.append(Image.open(os.path.join(sharp_folder_path,f)))

for f in os.listdir(blurry_folder_path):
    ext = os.path.splitext(f)[1]
    if ext.lower() not in valid_images:
        continue
    blurry_images.append(Image.open(os.path.join(blurry_folder_path,f)))


for i in sharp_images:
    temp_image = numpy.array(i)

    thresh = 35
    MinZero = 0.05

    x=numpy.asarray(temp_image)
